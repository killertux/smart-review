...
pub(crate) fn stored_analysis(head_sha: &str) -> crate::ports::StoredAnalysis {
    ...
    let normalized = crate::domain::analysis::normalize(
        r#"{"brief": "Billing rounds half up.", "inferred_purpose": "fix a bug",
            ...
            "per_file_notes": [{"path": "src/domain/money.rs", "what_changed": "rounding",
                                "why": "check the sign", "verify": ["negatives"],
                                "evidence": [{"path": "src/domain/money.rs", "side": "new",
                                              "line": 1, "label": "new rounding rule"}]}],
            "suggested_questions": ["Documented?"],
            "coverage": {"analyzed_files": ["src/domain/money.rs", "tests/money.rs"],
