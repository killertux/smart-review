...
SCREEN="$(run_tui "$HOME_MAIN" ':pr 141\r~ a~:q\r' 'worktree~Nothing has been sent yet~' "$FRAMES")"
...
: >"$TMP/requests.jsonl"
SCREEN="$(run_tui "$HOME_MAIN" ':pr 141\r~ a~ a~:q\r' 'worktree~Nothing has been sent yet~Money now rounds half up~' "$FRAMES")"
...
SCREEN="$(run_tui "$HOME_MAIN" ':pr 141\r~ a~:q\r' 'worktree~Money now rounds half up~' "$TMP/order.log")"
...
SCREEN="$(run_tui "$HOME_MAIN" ':pr 141\r~ a~\e~gg~e~m~:q\r' \
  'worktree~Money now rounds half up~~M src/domain/money\.rs~Evidence 1~✓M money\.rs~' \
  "$GUIDED_LOG")"
...
SCREEN="$(run_tui "$HOME_MAIN" ':pr 141\r~ a~\e~o~:q\r' 'worktree~Money now rounds half up~~path order~' "$TMP/order-toggle.log")"
...
LLM_PID=""
: >"$TMP/requests.jsonl"
SCREEN="$(run_tui "$HOME_MAIN" ':pr 141\r~ a~:q\r' 'worktree~Money now rounds half up~' "$TMP/cached.log")"
...
SCREEN="$(run_tui "$HOME_REPAIR" ':pr 141\r~ a~ a~:q\r' \
  'worktree~Nothing has been sent yet~This analysis needed a second attempt~' \
  "$TMP/repair.log")"
...
make_home "$HOME_BAD"
SCREEN="$(run_tui "$HOME_BAD" ':pr 141\r~ a~ a~:q\r' 'worktree~Nothing has been sent yet~could not be used~' "$TMP/bad.log")"
...
SCREEN="$(run_tui "$HOME_SMALL" ':pr 141\r~ a~ a~:context\r~:q\r' 'worktree~Nothing has been sent yet~Money now rounds half up~AGENTS\.md~' "$TMP/small.log")"
