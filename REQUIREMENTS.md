...
*Implementation note (M3):* a provider the pinned crate cannot stream for (DeepSeek, Groq, Mistral, OpenRouter — see Appendix B) still answers: the adapter walks down to the string stream, then to the OpenAI-compatible passthrough, then to a single un-streamed request, and says which of them answered in the log. Streaming is a `SHOULD`, and an answer that arrives whole is a better failure than a pane that waits forever.
...
- **Streaming is per backend, not per crate (verified at M3, `llm` 1.3.8).** Measured from the source: `chat_stream_struct` (text *and* usage) is implemented by `openai`, `google` and `azure_openai`; `chat_stream` (text only, no usage) by `anthropic`, `ollama` and `xai`; and **`deepseek`, `groq`, `mistral` and `openrouter` implement neither**, so for those the trait's default methods return `LLMError::Generic("…not supported for this provider")`. Asking every provider for the structured stream and reporting what came back is what made a DeepSeek user's question fail with the crate's own refusal. `adapters/llm.rs` therefore tries, in order: the structured stream, the string stream, the structured stream through the OpenAI-compatible passthrough (available when the catalog publishes a base URL — DeepSeek and OpenRouter do, which is how they get deltas *and* usage), and one request with no streaming (Groq, Mistral). A path that fails after emitting text ends the request, since that answer has been paid for.
