...
| IR-08 | `nix` | Signal a runner-owned Unix process group after cancellation or timeout | **Approved by owner during IR-08.** The standard library has no stable process-group API. `nix` provides safe Unix signal wrappers; Linux creates the group via its standard `setsid` launcher before the target program executes. |
