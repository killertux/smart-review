...
| IR-08 | `nix` | Signal a runner-owned Unix process group after cancellation or timeout | **Approved by owner during IR-08.** `std::process::CommandExt::process_group` creates the group before exec; `nix` provides safe Unix group-signal wrappers. |
